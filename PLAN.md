...
### APScheduler
- [ ] All jobs iterate `league_repo.get_active_leagues()`
- [ ] Auto-simulate jobs only run for leagues where `auto_simulate=True`
- [ ] Transfer window only enforced for `transfer_window_enabled=True`

### Seed service
- [ ] Keep `seed_sports()` (global, runs once)
- [ ] Add `seed_showcase_league()`: idempotent, creates showcase League + superadmin as creator
- [ ] Call both in `main.py` lifespan

### Demo seed endpoint
- [ ] `POST /leagues/{league_id}/admin/seed` — seeds specified league with demo data (idempotent)
...
- [ ] `loader`: fetch league detail, inject into route context
- [ ] `errorComponent`: "League not found"

### Auth guard updates
- [ ] `_admin.tsx` → `_superadmin.tsx`: check `session.role === "SUPERADMIN"`
- [ ] `_league_admin.tsx` (new): fetch `/leagues/{lid}/members/me`, check `LEAGUE_ADMIN`
- [ ] `_chief.tsx`: check league membership `role in ["LEAGUE_ADMIN", "CHIEF"]`

### Move existing routes
- [ ] All `(public)/` → `leagues/$leagueId/(public)/`
- [ ] All `_authenticated/dashboard/` → `leagues/$leagueId/_authenticated/dashboard/`
- [ ] `_authenticated/_admin/` → `leagues/$leagueId/_authenticated/_league_admin/`
- [ ] `_authenticated/_chief/` → `leagues/$leagueId/_authenticated/_chief/`
- [ ] Update all `Link`/`navigate` calls to include `leagueId`
...
### Type regeneration
- [ ] `bun run dev:backend` (backend must be running)
- [ ] `bun run --cwd apps/web gen:types`

### Query key factory (`src/queries/keys.ts`)
- [ ] All domain factories gain `leagueId` as first param
- [ ] Add `leagues` factory (no prefix)

### Query files to update
- [ ] NEW: `src/queries/leagues.ts`
- [ ] `competitions.ts` (was `weeks.ts`): add `leagueId`, URL `/leagues/${lid}/competitions`
- [ ] `delegations.ts`, `athletes.ts`, `events.ts`, `enrollments.ts`, `results.ts`, `reports.ts`, `ai.ts`: same pattern
- [ ] `activities.ts`, `search.ts`: add optional `leagueId`

### Mutation calls
- [ ] All `apiFetch` POSTs/PATCHes/DELETEs in league-scoped pages gain `/leagues/${leagueId}` prefix
...
- [ ] Fetch all leagues, show grid: name, sports count, member count, status
- [ ] Showcase league pinned with "Live" badge
- [ ] "Create your league" CTA (authenticated only)
- [ ] SSR

### Create league page (`src/routes/leagues/new.tsx`)
- [ ] Fields: name, slug (auto from name), description, timezone
- [ ] Sport checkboxes (10 options, min 1)
- [ ] Toggle: transfer window, auto-simulate
- [ ] Submit → redirect to `/leagues/{id}/dashboard`

### League settings (`leagues/$leagueId/_authenticated/_league_admin/settings/index.tsx`)
- [ ] Edit league config, sports selection, danger zone (archive)
- [ ] Member list with role management

### League dashboard (`leagues/$leagueId/_authenticated/dashboard/index.tsx`)
- [ ] LEAGUE_ADMIN: active competitions, pending requests, member count, AI shortcuts
- [ ] CHIEF: delegation overview, upcoming matches
- [ ] ATHLETE/COACH: upcoming matches, recent results
- [ ] `ssr: 'data-only'`

### My leagues page (`_authenticated/my-leagues/index.tsx`)
- [ ] User's leagues list with role per league
- [ ] Quick links + "Create new league"

---

## Phase 8 — Cleanup & Verification

- [ ] Remove old root-level public and dashboard routes
- [ ] Update `NotificationBell` / `Header` hardcoded links
- [ ] Notification payload gains `league_id`; links resolve to `/leagues/{lid}/...`
- [ ] `bun run check-types` — zero errors
- [ ] `bun run check` — zero lint/format errors
