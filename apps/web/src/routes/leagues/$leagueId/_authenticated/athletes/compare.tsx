...
export const Route = createFileRoute("/leagues/$leagueId/_authenticated/athletes/compare")({
...
function AthleteComparePage() {
  ...
  const { data: athleteList } = useSuspenseQuery(
    athleteListQueryOptions(Number(leagueId), { per_page: 500 }),
  );

  const leftQuery = useQuery({
    ...athleteReportQueryOptions(Number(leagueId), search.left ?? 0),
    enabled: Boolean(search.left),
  });
  const rightQuery = useQuery({
    ...athleteReportQueryOptions(Number(leagueId), search.right ?? 0),
        ...
        <Card className="border border-border/70 bg-[radial-gradient(circle_at_top_left,hsl(var(--primary)/0.16),transparent_40%),linear-gradient(160deg,hsl(var(--card)),hsl(var(--card)),hsl(var(--muted)/0.18))]">
          <CardHeader className="gap-3">
            ...
            <CardTitle className="text-3xl">
              Tela side-by-side para confronto de desempenho
            </CardTitle>
            <CardDescription className="max-w-2xl">
              Compare duas fichas de atleta usando histórico, medalhas e estatísticas agregadas da
              competição.
            </CardDescription>
          </CardHeader>
          <CardContent className="grid gap-4 md:grid-cols-3">
            ...
            <QuickMetric
              ...
              value={leftReport && rightReport ? "2 perfis" : "incompleta"}
            />
              ...
              <TableBody>
                {allMetricRows.map((row) => (
                  <TableRow key={row.label}>
                    <TableCell
                      className={
                        row.leftBetter ? "font-semibold text-foreground" : "text-muted-foreground"
                      }
                    >
                      {row.left}
                    </TableCell>
                    ...
                    <TableCell
                      className={`text-right ${row.rightBetter ? "font-semibold text-foreground" : "text-muted-foreground"}`}
                    >
                      {row.right}
                    </TableCell>
                  </TableRow>
...
function AthletePanel({ report, side }: { report: AthleteReportResponse | null; side: string }) {
  if (!report) {
    return (
      ...
      <CardHeader className="gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <CardDescription>
              {report.athlete.birthdate
                ? formatDate(report.athlete.birthdate)
      ...
      <CardContent className="space-y-6">
        <div className="grid gap-3 sm:grid-cols-2">
          <MiniStat
            ...
            label="Medalhas"
            value={String(snapshot.totalMedals)}
          />
          <MiniStat
            icon={<Users className="size-4" />}
            label="Delegações"
            value={String(snapshot.delegationCount)}
          />
          <MiniStat
            icon={<ArrowLeftRight className="size-4" />}
            label="Partidas"
            value={String(snapshot.totalMatches)}
          />
          <MiniStat
            icon={<Medal className="size-4" />}
            label="Esportes"
            value={String(snapshot.sportCount)}
          />
        </div>
