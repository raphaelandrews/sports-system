...
export const Route = createFileRoute("/leagues/$leagueId/_authenticated/dashboard/search")({
      ...
      <section className="grid gap-4 xl:grid-cols-[1.4fr_1fr]">
        <Card className="border border-border/70 bg-[radial-gradient(circle_at_top_left,hsl(var(--primary)/0.18),transparent_42%),linear-gradient(160deg,hsl(var(--card)),hsl(var(--card)),hsl(var(--muted)/0.22))]">
          <CardHeader className="gap-3">
            ...
            <CardTitle className="text-3xl">
              Localize entidades da competição em um único fluxo
            </CardTitle>
            <CardDescription className="max-w-2xl">
              Pesquisa unificada sobre atletas, delegações e eventos com acesso rápido para as áreas
              mais úteis do sistema.
            </CardDescription>
          </CardHeader>
        ...
        <Card className="border border-border/70">
          <CardHeader>
            ...
            <CardDescription>Estado atual da busca e volume de itens retornados.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3 text-sm">
            ...
            <SummaryRow
              ...
              value={
                query.length < 2
                  ? "Aguardando ao menos 2 caracteres"
        ...
        <div className="grid gap-6 xl:grid-cols-[1.05fr_0.95fr]">
          <div className="space-y-6">
            <Card className="border border-border/70">
              <CardHeader>
                ...
                <CardDescription>Busca por nome e código do atleta.</CardDescription>
              </CardHeader>
              <CardContent>
                <Table>
                  <TableHeader>
            ...
            <Card className="border border-border/70">
              <CardHeader>
                ...
                <CardDescription>Busca por nome e código da delegação.</CardDescription>
              </CardHeader>
              <CardContent>
                <Table>
                  <TableHeader>
          ...
          <Card className="border border-border/70">
            <CardHeader>
              ...
              <CardDescription>Busca por esporte, modalidade ou local do evento.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
              {results.events.map((event) => (
                <div key={event.id} className="rounded-2xl border border-border/70 bg-muted/25 p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <div className="text-sm text-muted-foreground">
                        Competição #{event.competition_number} · {formatDate(event.event_date)} ·{" "}
