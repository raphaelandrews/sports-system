...
export const Route = createFileRoute("/_authenticated/dashboard/results/$matchId/new")({
  ssr: false,
  loader: async ({ context: { queryClient }, params }) => {
    const matchId = Number(params.matchId);
    const match = await queryClient.ensureQueryData(matchDetailQueryOptions(matchId));
    const event = await queryClient.ensureQueryData(eventDetailQueryOptions(match.event_id));
    ...
    await Promise.all([
      ...
      queryClient.ensureQueryData(athleteListQueryOptions({ per_page: 500 })),
      queryClient.ensureQueryData(resultListQueryOptions({ per_page: 200 })),
...
function MatchResultEntryPage() {
  ...
  const { data: event } = useSuspenseQuery(eventDetailQueryOptions(match.event_id));
  const { data: allResults } = useSuspenseQuery(resultListQueryOptions({ per_page: 200 }));
  ...
  const [delegationId, setDelegationId] = useState(
    editable?.delegation_id ? String(editable.delegation_id) : match.team_a_delegation_id ? String(match.team_a_delegation_id) : "",
  );
  const [athleteId, setAthleteId] = useState(editable?.athlete_id ? String(editable.athlete_id) : "");
  const [valueJsonText, setValueJsonText] = useState(
    JSON.stringify(editable?.value_json ?? buildSuggestedValueJson(modality?.rules_json, sport?.stats_schema), null, 2),
  );
      ...
      <section className="grid gap-4 xl:grid-cols-[1.35fr_0.65fr]">
        <Card className="border border-border/70 bg-[radial-gradient(circle_at_top_left,hsl(var(--primary)/0.14),transparent_42%),linear-gradient(160deg,hsl(var(--card)),hsl(var(--card)),hsl(var(--muted)/0.20))]">
          <CardHeader className="gap-3">
            ...
            <CardTitle className="text-2xl">{sport?.name ?? "Esporte"} · {modality?.name ?? `Modalidade #${event.modality_id}`}</CardTitle>
            <CardDescription>
              {formatDate(event.event_date)} · {formatTime(event.start_time)} · status {match.status}
            </CardDescription>
          </CardHeader>
          <CardContent className="grid gap-4 md:grid-cols-3">
            <QuickBlock label="Delegação A" value={labelDelegation(match.team_a_delegation_id, delegationById)} />
            <QuickBlock label="Placar" value={match.score_a != null && match.score_b != null ? `${match.score_a} x ${match.score_b}` : "Sem placar"} />
            <QuickBlock label="Delegação B" value={labelDelegation(match.team_b_delegation_id, delegationById)} />
          </CardContent>
        </Card>
      ...
      <section className="grid gap-4 xl:grid-cols-[1fr_0.9fr]">
        <Card className="border border-border/70">
          <CardHeader>
            ...
            <CardDescription>
              Campos específicos do esporte entram em `value_json`.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <Button type="button" disabled={saveMutation.isPending} onClick={() => saveMutation.mutate()}>
              {saveMutation.isPending ? "Salvando..." : editable ? "Atualizar resultado" : "Criar resultado"}
            </Button>
          </CardContent>
        </Card>
        ...
        <Card className="border border-border/70">
          <CardHeader>
            ...
            <CardDescription>
              Itens já associados à partida.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            {existingResults.length > 0 ? (
              existingResults.map((result) => (
                <div key={result.id} className="rounded-3xl border border-border/70 bg-muted/15 p-4">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div className="font-medium">
                      {result.delegation_id ? labelDelegation(result.delegation_id, delegationById) : "Sem delegação"}
                    </div>
                    <Badge variant="outline">{result.medal ?? "Sem medalha"}</Badge>
                  </div>
                  <div className="mt-2 text-sm text-muted-foreground">
                    Rank {result.rank} · {result.athlete_id ? athleteById.get(result.athlete_id)?.name ?? `Atleta #${result.athlete_id}` : "Sem atleta"}
                  </div>
