...
export const Route = createFileRoute("/_authenticated/dashboard/ai/")({
  ...
  loader: ({ context: { queryClient } }) =>
    Promise.all([
      queryClient.ensureQueryData(aiGenerationHistoryQueryOptions()),
      queryClient.ensureQueryData(narrativeTodayQueryOptions()),
...
function AiControlRoomPage() {
  ...
  const { data: history } = useSuspenseQuery(aiGenerationHistoryQueryOptions());
  const { data: narrative } = useSuspenseQuery(narrativeTodayQueryOptions());
  ...
  const invalidateAiSurface = async () => {
    await Promise.all([
      ...
      queryClient.invalidateQueries({ queryKey: queryKeys.ai.narrative("today") }),
      queryClient.invalidateQueries({ queryKey: queryKeys.ai.narrative(narrativeDate) }),
    ]);
  };
        ...
        <Card className="overflow-hidden border border-border/70 bg-[radial-gradient(circle_at_top_left,hsl(var(--primary)/0.16),transparent_38%),linear-gradient(160deg,hsl(var(--card)),hsl(var(--card)),hsl(var(--muted)/0.22))]">
          <CardHeader className="gap-3">
            ...
            <CardDescription className="max-w-2xl">
              Dispare gerações por categoria, valide o preview antes de confirmar e acompanhe o histórico consolidado da automação.
            </CardDescription>
          </CardHeader>
          <CardContent className="grid gap-4 md:grid-cols-4">
            <HeroStat label="Delegações" value={String(delegations.data.length)} hint="bases disponíveis" />
            <HeroStat label="Atletas" value={String(athletes.meta.total)} hint="cadastros atuais" />
            <HeroStat label="Inscrições" value={String(enrollments.meta.total)} hint="pipeline ativo" />
            <HeroStat label="Resultados" value={String(results.meta.total)} hint="registros emitidos" />
          </CardContent>
        </Card>
        ...
        <Card className="border border-border/70 bg-[linear-gradient(180deg,hsl(var(--card)),hsl(var(--muted)/0.18))]">
          ...
          <CardContent className="space-y-4">
            <div className="rounded-3xl border border-border/70 bg-background/75 p-4">
              <div className="text-[11px] uppercase tracking-[0.24em] text-muted-foreground">Narrativa atual</div>
              ...
              <div className="mt-2 line-clamp-6 whitespace-pre-wrap text-sm text-muted-foreground">
                {narrative?.content ?? "Use o card de narrativa para gerar o primeiro texto editorial do dia."}
              </div>
            </div>
            <div className="rounded-3xl border border-border/70 bg-background/75 p-4">
              <div className="text-[11px] uppercase tracking-[0.24em] text-muted-foreground">Próximo evento para resultados</div>
              <div className="mt-2 text-sm font-medium">
                {selectedEvent
      ...
      <section className="grid gap-4 2xl:grid-cols-[1.35fr_0.65fr]">
        <div className="grid gap-4 md:grid-cols-2">
            ...
            footer={
              <AiGenerateButton
                ...
                previewItems={[
                  ...
                  { label: "Impacto", value: "Novas delegações ficarão disponíveis no dashboard e no ranking" },
                ]}
                successMessage={(data: { length: number }) => `${data.length} delegações geradas com IA.`}
            ...
            footer={
              <AiGenerateButton
                ...
                previewItems={[
                  ...
                  { label: "Impacto", value: "Novas modalidades poderão ser vinculadas nas próximas fases" },
                ]}
                successMessage={(data: { length: number }) => `${data.length} esportes gerados com IA.`}
          ...
          <GeneratorCard
            ...
            controls={<MetricStrip label="Base atual" value={`${athletes.meta.total} atletas cadastrados`} />}
            footer={
              <AiGenerateButton
                label="Gerar atleta"
          ...
          <GeneratorCard
            ...
            controls={<MetricStrip label="Fila atual" value={`${enrollments.meta.total} inscrições`} />}
            footer={
              <AiGenerateButton
                ...
                previewItems={[
                  ...
                  { label: "Impacto", value: "O backend seguirá a validação oficial antes de persistir" },
                ]}
                successMessage={(data: { length: number }) => `${data.length} inscrições geradas com IA.`}
          ...
          <GeneratorCard
                  ...
                  <SelectContent>
                    {candidateEvents.map((event) => (
                      <SelectItem key={event.id} value={String(event.id)}>
                        Evento #{event.id} · {formatDate(event.event_date)} · {formatTime(event.start_time)}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
            ...
            footer={
              <AiGenerateButton
                ...
                previewItems={[
                  { label: "Endpoint", value: selectedEvent ? `/results/ai-generate/${selectedEvent.id}` : "/results/ai-generate/:event_id" },
                  { label: "Evento", value: selectedEvent ? `#${selectedEvent.id}` : "Selecione um evento" },
                  { label: "Agendamento", value: selectedEvent ? `${formatDate(selectedEvent.event_date)} · ${formatTime(selectedEvent.start_time)}` : "Sem agendamento" },
                  { label: "Impacto", value: "Quadro de medalhas e standings podem ser atualizados" },
                ]}
                disabled={!selectedEvent}
                successMessage={(data: { length: number }) => `${data.length} resultados gerados com IA.`}
            ...
            footer={
              <AiGenerateButton<NarrativeResponse>
                ...
                previewItems={[
                  ...
                  { label: "Data", value: narrativeDate ? formatDate(narrativeDate) : "Selecione uma data" },
                  { label: "Narrativa atual", value: narrative ? formatEventDate(narrative.generated_at, { dateStyle: "medium", timeStyle: "short" }) : "Nenhuma geração anterior" },
                  { label: "Impacto", value: "Atualiza o texto editorial consumido no painel de narrativa" },
                ]}
                successMessage={(data) => `Narrativa gerada para ${formatDate(data.narrative_date)}.`}
        ...
        <div className="space-y-4">
          <Card className="border border-border/70">
            <CardHeader>
              ...
              <CardDescription>
                Últimas execuções registradas pelo backend.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <ScrollArea className="h-[420px] pr-4">
                <div className="space-y-3">
                  {history.map((entry) => (
                    <div key={entry.id} className="rounded-3xl border border-border/70 bg-muted/15 p-4">
                      <div className="flex items-center justify-between gap-3">
                        <Badge variant="outline">{entry.generation_type}</Badge>
                        <div className="text-xs text-muted-foreground">
                          {formatEventDate(entry.created_at, { dateStyle: "medium", timeStyle: "short" })}
                        </div>
                      </div>
                      <div className="mt-3 text-lg font-semibold">{entry.count}</div>
                      <div className="text-sm text-muted-foreground">item(ns) gerados nessa execução</div>
                    </div>
                  ))}
                  {history.length === 0 ? (
