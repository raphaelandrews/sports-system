...
export const Route = createFileRoute("/_authenticated/dashboard/enrollments/new")({
  ...
  loader: ({ context: { queryClient } }) => {
    ...
    void queryClient.prefetchQuery(enrollmentListQueryOptions({ per_page: 200 }))
...
function NewEnrollmentPage() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      await navigate({ to: "/dashboard/enrollments" });
    },
    onError: (error) => {
                      ...
                      {selectableEvents.map((event) => {
                        ...
                        return (
                          <SelectItem key={event.id} value={String(event.id)}>
                            {modality?.name ?? `Evento #${event.id}`} · {sport?.name ?? "Esporte"} · {formatDate(event.event_date)} · {formatTime(event.start_time)}{locked ? " · bloqueado" : ""}
                          </SelectItem>
                        );
                      })}
                    </SelectContent>
            ...
            <div className="grid gap-4 xl:grid-cols-[1.15fr_0.85fr]">
              <Card className="border border-border/70">
                ...
                <CardContent className="space-y-3 text-sm text-muted-foreground">
                  ...
                  />
                  <SummaryRow label="Atleta" value={selectedAthlete ? `${selectedAthlete.name} · ${selectedAthlete.code}` : "—"} />
                </CardContent>
              </Card>
                ...
                <CardContent className="space-y-3">
                  ...
                  {localValidation ? (
                    <Alert variant={localValidation.type === "error" ? "destructive" : "default"}>
                      <AlertTitle>
                        {localValidation.type === "success" ? "Pré-validação ok" : "Ajuste necessário"}
                      </AlertTitle>
