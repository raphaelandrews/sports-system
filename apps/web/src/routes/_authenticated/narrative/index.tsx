...
function NarrativePage() {
  ...
  const { data: todayNarrative } = useSuspenseQuery(narrativeTodayQueryOptions());
  ...
  const narrativeQuery = useQuery({
    queryKey: queryKeys.ai.narrative(selectedDate),
    queryFn: async () => {
      try {
  ...
  const activeNarrative = narrativeQuery.data ?? (todayNarrative?.narrative_date === selectedDate ? todayNarrative : null);
        ...
        <Card className="border border-border/70 bg-[radial-gradient(circle_at_top_left,hsl(var(--primary)/0.14),transparent_38%),linear-gradient(160deg,hsl(var(--card)),hsl(var(--card)),hsl(var(--muted)/0.20))]">
          <CardHeader className="gap-3">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Badge variant="secondary">{isAdmin ? "Admin pode gerar" : "Leitura autenticada"}</Badge>
            </div>
            <CardTitle className="text-3xl">Crônica diária da competição</CardTitle>
            <CardDescription className="max-w-2xl">
              Acompanhe o histórico editorial por dia e abra a narrativa consolidada com destaque visual.
            </CardDescription>
          </CardHeader>
        ...
        <Card className="border border-border/70">
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2">
              <div className="text-[11px] uppercase tracking-[0.24em] text-muted-foreground">Data</div>
              <Input
                type="date"
                value={selectedDate}
            ...
            {isAdmin ? (
              <AiGenerateButton<NarrativeResponse>
                ...
                previewItems={[
                  ...
                  { label: "Impacto", value: "Substitui a versão existente da mesma data, se houver" },
                ]}
                ...
                onSuccess={async () => {
                  await Promise.all([
                    ...
                    queryClient.invalidateQueries({ queryKey: queryKeys.ai.narrative(selectedDate) }),
                  ]);
                }}
              />
            ) : null}
