...
function AthleteProfilePage() {
  ...
  const { data } = useSuspenseQuery(
    athleteReportQueryOptions(Number(athleteId)),
  );
                ...
                <TableBody>
                  {delegation_history.map((item, i) => (
                    <TableRow key={i}>
                      ...
                      <TableCell>
                        {item.left_at
                          ? formatDate(item.left_at.slice(0, 10))
                          : "Atual"}
                      </TableCell>
                    </TableRow>
                  ))}
                ...
                <TableBody>
                  {match_history.map((item) => (
                    <TableRow key={item.match_id}>
                      ...
                      <TableCell>
                        {item.match_date
                          ? formatDate(item.match_date.slice(0, 10))
                          : "—"}
                      </TableCell>
                    </TableRow>
                  ))}
          ...
          {Object.keys(statistics).length === 0 ? (
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {Object.entries(statistics).map(([key, stats]) => {
                const label = key
                  .replace("sport_", "Esporte ")
                  .replace("_week_", " · Semana ");
                return (
                  <Card key={key}>
