...
export const Route = createFileRoute("/_authenticated/matches/$matchId/")({
  ...
  loader: async ({ context: { queryClient }, params }) => {
    ...
    await Promise.all([
      queryClient.ensureQueryData(matchDetailQueryOptions(matchId)),
...
function MatchLivePage() {
  ...
  const { data: athletes } = useSuspenseQuery(athleteListQueryOptions({ per_page: 500 }));
              ...
              match.participants.map((participant) => (
                ...
                >
                  <div className="font-medium">
                    {athleteById.get(participant.athlete_id)?.name ?? `Atleta #${participant.athlete_id}`}
                  </div>
                  <div className="text-sm text-muted-foreground">
                    {labelDelegation(participant.delegation_id_at_time, delegationById)} · {participant.role}
                  </div>
                </div>
              ))
            ) : (
      ...
      <section className="grid gap-4 xl:grid-cols-[1fr_0.95fr]">
                ...
                .map((event) => (
                  ...
                  >
                    ...
                    <div className="space-y-1">
                      <div className="font-medium">
                        {eventTypeOptions.find((option) => option.value === event.event_type)?.label ??
                          event.event_type}
                      </div>
                      <div className="text-sm text-muted-foreground">
                        {event.athlete_id
                          ? athleteById.get(event.athlete_id)?.name ?? `Atleta #${event.athlete_id}`
                          : "Sem atleta"}
                        {" · "}
        ...
        <Card className="border border-border/70">
          <CardHeader>
            ...
            <CardDescription>
              Inicio, encerramento e registro manual de eventos.
            </CardDescription>
          </CardHeader>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <Field>
            ...
            >
              ...
              <SelectContent>
                {Array.from(new Set(participants.map((participant) => participant.delegation_id_at_time))).map(
                  (delegationId) => (
