...
export const Route = createFileRoute("/(public)/results/sports/$sportId/")({
  loader: async ({ context: { queryClient }, params }) => {
    const sportId = Number(params.sportId);
    const sport = await queryClient.ensureQueryData(sportDetailQueryOptions(sportId));

    await Promise.all([
      queryClient.ensureQueryData(sportMedalBoardQueryOptions(sportId)),
      ...sport.modalities.map((modality) =>
        queryClient.ensureQueryData(modalityStandingsQueryOptions(modality.id)),
      ),
    ]);
  },
...
function SportResultsPage() {
  ...
  const { data: medalBoard } = useSuspenseQuery(sportMedalBoardQueryOptions(numericSportId));
  const standings = useSuspenseQueries({
    queries: sport.modalities.map((modality) => modalityStandingsQueryOptions(modality.id)),
  });
