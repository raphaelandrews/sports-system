...
async def list_activity_feed(
    ...
    return await activity_service.list_feed(session, limit)


@router.get("/stream")
async def stream_activity_feed(request: Request) -> EventSourceResponse:
    async def generator() -> AsyncGenerator[str, None]:
        q = sse.subscribe_activity_feed()
        try:
            while True:
                data = await q.get()
                yield data
            ...
            sse.unsubscribe_activity_feed(q)
