...
async def get_today(session: AsyncSession = Depends(get_session)) -> NarrativeResponse | None:
    narrative = await narrative_service.get_today(session)
...
async def get_for_date(narrative_date: date, session: AsyncSession = Depends(get_session)) -> NarrativeResponse:
    narrative = await narrative_service.get_for_date(session, narrative_date)
    if narrative is None:
...
async def generate_narrative(
    ...
    narrative = await narrative_service.generate(session, target_date)
    return NarrativeResponse.model_validate(narrative)
...
async def get_generation_history(
    ...
    records = await narrative_service.get_history(session)
