...
async def list_recent_match_events_global(
...
async def list_recent_match_events(
...
async def list_recent_match_state_changes_global(
    session: AsyncSession, limit: int
) -> list[dict[str, Any]]:
...
async def list_recent_match_state_changes(
    session: AsyncSession, league_id: int, limit: int
) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
...
async def list_recent_records_global(
    session: AsyncSession, limit: int
) -> list[dict[str, Any]]:
    result = await session.execute(
        select(
