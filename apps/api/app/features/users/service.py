...
from app.features.users import chief_repository as chief_request_repository
from app.features.auth import user_repository
from app.domain.schemas.user import ChiefRequestCreate, ChiefRequestReview, UserUpdate
...
async def request_chief(
    session: AsyncSession, user: User, data: ChiefRequestCreate
) -> ChiefRequest:
    req = ChiefRequest(
        user_id=user.id,
...
async def review_request(
    ...
    if data.status == ChiefRequestStatus.APPROVED:
        ...
        code = (
            req.delegation_name[:4].upper().replace(" ", "")
