...
def _phase_medals(
    phase: EventPhase,
) -> tuple[int, Optional[Medal], int, Optional[Medal]]:
...
async def _check_and_update_record(
    ...
    existing = await result_repository.get_best_record_for_modality(
        session, modality.id
    )
...
async def simulate_match(session: AsyncSession, match_id: int) -> None:
    ...
    if lower_wins:
        winner_id = (
            ...
            if score_a < score_b
            else match.team_b_delegation_id
        )
    ...
    if winner_id is not None:
        ...
        session.add(
            Result(
                match_id=match.id,
                delegation_id=match.team_a_delegation_id,
                rank=1,
                value_json={"score": score_a},
            )
        )
        session.add(
            Result(
                match_id=match.id,
                delegation_id=match.team_b_delegation_id,
                rank=1,
                value_json={"score": score_b},
            )
        )
    ...
    if competition is not None:
        await sse.broadcast_medal_board(
            competition.league_id, {"type": "medal_board_updated", "match_id": match_id}
        )
