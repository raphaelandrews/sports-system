...
from app.domain.schemas.league import LeagueCreate, LeagueMemberRoleUpdate, LeagueUpdate
...
async def create_league(
    session: AsyncSession, data: LeagueCreate, creator: User
) -> League:
    existing = await league_repository.get_by_slug(session, data.slug)
    if existing is not None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT, detail="League slug already exists"
        )
    ...
    if data.slug is not None and data.slug != league.slug:
        ...
        if existing is not None and existing.id != league.id:
            raise HTTPException(
                ...
                detail="League slug already exists",
            )
...
async def archive_league(
    session: AsyncSession, league_id: int, current_user: User
) -> League:
    league = await get_league_or_404(session, league_id)
...
async def get_member_or_403(
    session: AsyncSession, league_id: int, user: User
) -> LeagueMember:
...
async def require_league_admin(
    ...
    if member.role != LeagueMemberRole.LEAGUE_ADMIN:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="League admin role required"
        )
    return member
...
async def add_member(
    ...
    if existing is not None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="User is already a league member",
        )
...
async def update_member_role(
    ...
    if member is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="League member not found"
        )
...
async def remove_member(
    ...
    if member.role == LeagueMemberRole.LEAGUE_ADMIN and not _is_superadmin(
        current_user
    ):
        admin_members = [
            item
            for item in await league_repository.get_members(session, league.id)
            if item.role == LeagueMemberRole.LEAGUE_ADMIN
        ]
