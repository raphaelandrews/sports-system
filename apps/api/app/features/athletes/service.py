...
async def create_athlete(
    ...
    if existing is not None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT, detail="Athlete code already in use"
        )
    athlete = Athlete(
        league_id=league_id,
        name=data.name,
