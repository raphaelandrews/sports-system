...
from app.features.enrollments import repository as enrollment_repository
from app.features.delegations.repository import get_current_delegation_id
...
from app.features.enrollments import eligibility as eligibility_service
...
async def list_enrollments(
    ...
    if membership.role == LeagueMemberRole.LEAGUE_ADMIN:
        ...
        chief_delegation_id = await get_current_delegation_id(
            session, current_user_id, league_id
        )
        if chief_delegation_id is None:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="User has no delegation"
            )
...
async def create_enrollment(
    ...
    if not await enrollment_repository.delegation_in_league(
        session, league_id, data.delegation_id
    ):
        raise HTTPException(
    ...
    if competition is None or competition.status in (
    ...
    existing = await enrollment_repository.get_by_athlete_and_event(
        session, data.athlete_id, data.event_id
    )
    if existing is not None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Athlete already enrolled in this event",
        )

    await _validate(
        session, athlete, event, modality, sport, competition, data.delegation_id
    )
...
async def cancel_enrollment(
    ...
    enrollment = await enrollment_repository.get_by_id_in_league(
        session, league_id, enrollment_id
    )
    ...
    if event is not None:
        competition = await session.get(Competition, event.competition_id)
        if competition is not None and competition.status in (
            CompetitionStatus.LOCKED,
            CompetitionStatus.ACTIVE,
            CompetitionStatus.COMPLETED,
        ):
...
async def ai_generate(
    ...
    await session.commit()
    logger.info(
        "ai_generate_enrollments created=%s competition_id=%s",
        len(created),
        competition.id,
    )
    return created
