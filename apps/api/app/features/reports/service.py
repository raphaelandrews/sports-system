...
from app.domain.models.athlete import AthleteModality
from app.domain.models.delegation import Delegation
from app.domain.models.event import Event, Match, MatchStatus
from app.domain.models.result import AthleteStatistic, Result
...
from app.domain.schemas.athlete import (
    AthleteResponse,
...
from app.domain.schemas.result import MedalBoardEntry, ResultResponse
...
async def get_final_report(
    ...
    total_athletes = (
        await session.execute(
            select(func.count())
            .select_from(Athlete)
            .where(Athlete.league_id == league_id, Athlete.is_active == True)
        )
    ).scalar_one()  # noqa: E712
    total_competitions = (
        await session.execute(
    ...
    athletes_by_sport_result = await session.execute(
        select(
            Sport.id,
            Sport.name,
            func.count(func.distinct(AthleteModality.athlete_id)).label(
                "athlete_count"
            ),
        )
        .select_from(Sport)
        ...
        .order_by(
            func.count(func.distinct(AthleteModality.athlete_id)).desc(), Sport.name
        )
    )
...
async def get_competition_report(
    ...
    competition = await session.execute(
        select(Competition).where(
            Competition.id == competition_id, Competition.league_id == league_id
        )
    )
...
async def get_athlete_report(
    ...
    athlete = await athlete_repository.get_by_id_in_league(
        session, league_id, athlete_id
    )
    if athlete is None:
        raise HTTPException(
    ...
    statistics = {
        f"sport_{s.sport_id}_competition_{s.competition_id}": s.stats_json
        for s in raw_stats
    }
