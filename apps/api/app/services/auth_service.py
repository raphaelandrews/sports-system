...
async def complete_oauth(session: AsyncSession, provider: str, code: str, state: str) -> str:
    config = _get_provider_config(provider)
    _ensure_provider_configured(config)
...
async def finalize_oauth(session: AsyncSession, relay_token: str) -> TokenResponse:
    try:
        ...
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid OAuth token") from exc

    user_id = payload.get("sub")
    if not isinstance(user_id, str) or not user_id.isdigit():
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid OAuth token payload")
...
async def _fetch_oauth_profile(config: OAuthProviderConfig, code: str) -> dict[str, str]:
    async with httpx.AsyncClient(timeout=20.0) as client:
        if config.provider == "google":
            ...
            if not email:
                raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="Google account has no email")
        ...
        if not email:
            raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="GitHub account has no public or primary email")
...
async def _get_or_create_oauth_user(session: AsyncSession, email: str, name: str) -> User:
    user = await user_repository.get_by_email(session, email)
    if user is not None:
        if not user.is_active:
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Account deactivated")
